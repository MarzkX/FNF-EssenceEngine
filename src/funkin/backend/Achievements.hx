...
class Achievements {
	...
	public static function load():Void
	{
		...
		if(ST.award().data != null) {
			if(ST.award().data.achievementsUnlocked != null)
				achievementsUnlocked = ST.award().data.achievementsUnlocked;

			var savedMap:Map<String, Float> = cast ST.award().data.achievementsVariables;
			if(savedMap != null)
			{
	...
	{
		ST.award().data.achievementsUnlocked = achievementsUnlocked;
		ST.award().data.achievementsVariables = variables;
		ST.award().flush();
	}
	...
	static function _scoreFunc(name:String, mode:Int = 0, addOrSet:Float = 1, saveIfNotUnlocked:Bool = true):Float
	{
		...
		if(achievements.exists(name))
		{
			...
			Achievements.save();
			if(saveIfNotUnlocked || val >= achievement.maxScore) ST.award().flush();
			return val;
		}
		return -1;
	}
	...
	@:allow(funkin.ui.awards.AchievementPopup)
	...
	public static function reloadList()
	{
		...
		loadAchievementJson(Paths.mods('data/jsonData/achievements.json'));
