...
class RGBUtil
{
    public static function int_desat(col:FlxColor, sat:Float) { //except this one
        var hsv = rgb2hsv(int2rgb(col));
        hsv.saturation *= (1 - sat);
        var rgb = hsv2rgb(hsv);
        return FlxColor.fromRGBFloat(rgb.red, rgb.green, rgb.blue);
    }
    public static function int2rgb(col:Dynamic) return {red: (col >> 16) & 0xff, green: (col >> 8) & 0xff, blue: col & 0xff}; //and this one
