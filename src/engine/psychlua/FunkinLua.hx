...
import engine.psychlua.LuaUtils.LuaTweenOptions;
#if SScript
...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		set('song_camMove', PlayState.SONG.camMove);
		set('song_camPixels', PlayState.SONG.camPixels);
		set('song_camSmooth', PlayState.SONG.camSmooth);
		set('song_camSmoothIntensity', PlayState.SONG.camSmoothIntensity);
		set('song_charTrails', PlayState.SONG.charTrails);
		...
		Lua_helper.add_callback(lua, "addHScript", function(luaFile:String, ?ignoreAlreadyRunning:Bool = false) {
			#if HSCRIPT_ALLOWED
			var foundScript:String = findScript(luaFile, '.hxc');
			if(foundScript != null)
			{
				if(!ignoreAlreadyRunning)
		...
		Lua_helper.add_callback(lua, "loadSong", function(?name:String = null, ?difficultyNum:Int = -1) {
			...
			EssenceFlxG.loading(new PlayState(), 'Game');
		...
		Lua_helper.add_callback(lua, "exitSong", function(?skipTransition:Bool = false) {
			...
			if(PlayState.isStoryMode)
				EssenceFlxG.switchState(() -> new StoryMenuState());
			else
