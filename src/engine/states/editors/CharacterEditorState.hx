...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		healthBar = new FlxSprite(31, FlxG.height - 76).makeGraphic(600, 18, FlxColor.WHITE);
		healthBar.scrollFactor.set();
		add(healthBar);
		healthBar.cameras = [camHUD];

		var healthBarBG:FlxSprite		= new FlxSprite(30, FlxG.height - 75).loadGraphic(Paths.image('healthBar'));
	...
	function addSettingsUI() {
		...
		{
			var parsedJson:CharacterFile = cast tjson.TJSON.parse(TemplateCharacter);
	...
	function addCharacterUI() {
		...
		tab_group.add(new FlxText(15, vocalsInputText.y - 18, 0, 'Vocals File:'));
	...
	function reloadCharacterImage()
	{
		...
		else if(Paths.fileExists('images/' + char.imageFile + '.json', TEXT)) char.frames = Paths.getAsepriteAtlas(char.imageFile);
