...
import engine.states.editors.ConfirmSubState;
...
class CharacterEditor extends MusicBeatState
{
	...
	var ghost:FlxSprite;
	var animateGhost:FlxAtlasSprite;
	...
	override function create()
	{
		...
		var dad:FlxSprite = new FlxSprite(dadPosition.x, dadPosition.y).loadGraphic(Paths.image('ui/editors/silhouetteDad'));
		dad.antialiasing = ClientPrefs.data.antialiasing;
		dad.active = false;
		dad.offset.set(-4, 1);
		silhouettes.add(dad);

		var boyfriend:FlxSprite = new FlxSprite(bfPosition.x, bfPosition.y + 350).loadGraphic(Paths.image('ui/editors/silhouetteBF'));
		...
		var makeGhostButton:FlxButton = new FlxButton(25, 15, "Make Ghost", function() {
			...
			if(!character.isAnimationNull())
			{
				...
				else if(myAnim != null) //This is VERY unoptimized and bad, I hope to find a better replacement that loads only a specific frame as bitmap in the future.
				{
					...
					{
						animateGhost = new FlxAtlasSprite(ghost.x, ghost.y, Paths.getPath('images/'+character.imageFile, TEXT, null, true));
	...
	function addSettingsUI()
	{
		...
		charDropDown = new FlxUIDropDownMenu(10, 30, FlxUIDropDownMenu.makeStrIdLabelArray([''], true), function(index:String)
		{
			...
			var characterPath:String = 'data/charData/$intended.json';
	...
	function reloadCharacterImage()
	{
		...
		{
			character.atlas = new FlxAtlasSprite(0, 0, Paths.getPath('images/'+character.imageFile, TEXT, null, true));
	...
	override function update(elapsed:Float)
	{
		...
		else if(FlxG.keys.justPressed.ESCAPE)
		{
			...
			if(!_goToPlayState)
			{
				...
				else openSubState(new ConfirmSubstate());
			}
			else loading(new PlayState(), 'Game');
			return;
		}
	}
	...
	inline function updatePointerPos()
	{
		var offX:Float = 0;
		var offY:Float = 0;
	...
	function reloadCharacterDropDown() {
		...
		var foldersToCheck:Array<String> = Mods.directoriesWithFile(Paths.getSharedPath(), 'data/charData/');
