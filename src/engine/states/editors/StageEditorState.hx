...
class StageEditorState extends MusicBeatState
{
	...
	function addDataTab()
	{
		...
		#if sys
		for (folder in FileSystem.readDirectory('funkin/'))
			if(FileSystem.isDirectory('funkin/$folder') && !Mods.ignoreModFolders.contains(folder))
	...
	function addStageTab()
	{
		...
		{
			var characterPath:String = 'data/stageData/$selected.json';
	...
	function reloadStageDropDown()
	{
		...
		var foldersToCheck:Array<String> = Mods.directoriesWithFile(Paths.getSharedPath(), 'data/stageData/');
		for (folder in foldersToCheck)
            #if sys
			for (file in FileSystem.readDirectory(folder))
				if(file.toLowerCase().endsWith('.json'))
	...
	private function onLoadComplete(_):Void
	{
		...
		if(fullPath != null)
		{
			...
			if(fullPath.startsWith(exePath + 'funkin/images/') #if MODS_ALLOWED || (fullPath.startsWith(exePath + 'addons/') && fullPath.contains('/images/')) #end)
			{
