...
class ModsMenuState extends MusicBeatState
{
	...
	function reload()
	{
		...
		var curMod:ModItem = modsGroup.members[curSelectedMod];
		state(new ModsMenuState(curMod != null ? curMod.folder : null));
	}
	...
	function saveTxt()
	{
		...
		#if sys
		var path:String = 'bin/modsList.txt';
		File.saveContent(path, fileStr);
		#else
		trace('yooo systeeem is no ssyyyssteeem)))))))))');
		#end
	}
}
...
class ModItem extends FlxSpriteGroup
{
	...
	public function new(folder:String)
	{
		...
		#if MODS_ALLOWED
		var path:String = Paths.mods('$folder/data/jsonData/settings.json');
